...
export interface ISpeechToTextWidget {
  ...
  onGetTranscriptions?: (output: any) => void;
}
...
export const SpeechToTextWidget = (props: ISpeechToTextWidget) => {
  ...
  useEffect(() => {
    if(transcriptions.length > 0) {
      ...
      props.onChangeOutput && props.onChangeOutput(text)
      setTranscriptions([])
      ...
      navigator.mediaDevices.getUserMedia({ audio: true }).then(stream => {
        ...
        socket.onmessage = (message) => {
          ...
          if(transcript.length > 0) {
            ...
            console.log(startMs, durationMs, endMs, endMs - startMs)

            props.onGetTranscriptions && props.onGetTranscriptions({
              transcript, startMs, durationMs, endMs
            })
          } else {
