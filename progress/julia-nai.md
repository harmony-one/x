2023-12-01 Fri: Tested the app and identified some issues with custom mode selector. There are multiple instances of the custom mode text in the app, so I am working on identifying which are still needed and which are extraneous. The original implimentation of custom instructions through the system seems does not seem to be connected entirely so overriding that with custom modes does not produce the intended results. Worked on getting local build working again with new API key.
